...
int	COXParserElement::NumElement() const
{
	int iEleCount = 0;
	for (int i = 0;i < m_Objects.size();i++)
	{
		COXParserObject * pObject = m_Objects[i];
		if(COXParserObject::ELEMENT == pObject->GetType())
			iEleCount++;
	}
	return iEleCount;
}

COXParserElement * COXParserElement::Object_Ele(int nIndex) const 
{
	int iEleCount = 0;
	for (int i = 0;i < m_Objects.size();i++)
	{
		...
		{
			if(iEleCount == nIndex)
				return (COXParserElement*)pObject;
			iEleCount++;
		}
	}
	return NULL;
...
LPCTSTR COXParserElement::GetElementValue() const
{
	for(int i = 0;i < m_Objects.size();i++)
	{
		COXParserObject * pObject = m_Objects[i];
		if(PLAINTEXT == pObject->GetType())
			return m_Objects[i]->GetText();
	}
	return NULL;
}
...
LPCTSTR COXParserElement::GetAttributeValue(LPCTSTR szName, bool bCaseSensitive)
{
	if (!szName)
		return NULL;
	...
	for (int i = 0; i < NumAttributes(); i++)
	{
		...
		if (pAttribute->IsName(szName, (BOOL) bCaseSensitive))
		{
			return pAttribute->GetStringValue();
		}
	}
	return NULL;
}
...
BOOL COXParser::Parse_C(LPCTSTR ptr)
{
	int iBufferSize = _tcslen(ptr) * sizeof(TCHAR);

	LPTSTR temBuf = new TCHAR[_tcslen(ptr) + 1];
	memset(temBuf,0,iBufferSize + sizeof(TCHAR) );
	memcpy(temBuf,ptr,iBufferSize);
...
{   
	...
	CString strText = pObject->GetText() ;
...
{   
    ...
    while (*ptr)
    {
		...
		/// 
		CString strContent = (LPTSTR)strLine.GetString() ;

		int iOrgLength = strContent.GetLength() ; 
		if ( iOrgLength < 1 )
		{
			return FALSE;
		}

		LPSTR lpsContent = T2A( strContent ) ; 

		int iLength = strlen( lpsContent ) ; 
...
//{
//	CString strFileCont ;
//	//pmcommon::GetFileContent( szFile , strFileCont ) ;
//
//	return (LPTSTR)(LPCTSTR)strFileCont ;
//}
